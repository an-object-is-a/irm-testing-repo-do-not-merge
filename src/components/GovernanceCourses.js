...
const GovernanceCourses = () => {
  /*
   Getting all Wordpress blog posts with the 'category' of "Governance".
  */
  const data = useStaticQuery(graphql`
    query GovernanceCourses {
      allWpPost(
        filter: {categories: {nodes: {elemMatch: {name: {eq: "Governance"}}}}}
      ) {
        edges {
          node {
            title
...
export default GovernanceCourses;