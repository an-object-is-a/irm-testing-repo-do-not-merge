...
const CybersecurityCourses = () => {
  /*
   Getting all Wordpress blog posts with the 'category' of "cybersecurity".
  */
  const data = useStaticQuery(graphql`
    query CybersecurityCourses {
      allWpPost(
        filter: {categories: {nodes: {elemMatch: {name: {eq: "cybersecurity"}}}}}
      ) {
        edges {
          node {
            title
    ...
    <>
      {data.allWpPost.edges.map(({ node }, index) => (
        <tr key={index}>
          ...
          <td width="15%" className="center"><a target="_blank" href={node.cybersecurity.downloadLink1.url}><img width="30px" src={download_icon} alt="IRM Consulting & Advisory | Your Cybersecurity Trusted Advisor" /></a></td>
        </tr>
      ))}
    </>
  )
