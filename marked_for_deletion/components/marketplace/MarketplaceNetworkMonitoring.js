...
const MarketplaceNetworkMonitoring = () => {
  ...
  return (
    <StaticQuery
      query={graphql`
        query MarketplaceNetworkMonitoring {
          allWpPost(
            filter: {categories: {nodes: {elemMatch: {name: {in: "Network Monitoring"}}}}}
          ) {
            edges {
              node {
                title
...
export default MarketplaceNetworkMonitoring;
