...
const MarketplaceThreatModeling = () => {
  ...
  return (
    <StaticQuery
      query={graphql`
        query MarketplaceThreatModeling {
          allWpPost(
            filter: {categories: {nodes: {elemMatch: {name: {in: "Threat Modeling"}}}}}
          ) {
            edges {
              node {
                title
...
export default MarketplaceThreatModeling;
