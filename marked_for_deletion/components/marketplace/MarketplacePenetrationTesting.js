...
const MarketplacePenetrationTesting = () => {
  ...
  return (
    <StaticQuery
      query={graphql`
        query MarketplacePenetrationTesting {
          allWpPost(
            filter: {categories: {nodes: {elemMatch: {name: {in: "Penetration Testing"}}}}}
          ) {
            edges {
              node {
                title
...
export default MarketplacePenetrationTesting;
